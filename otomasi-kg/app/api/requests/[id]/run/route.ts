...
export async function POST(
  ...
  try {
    ...
    if (request.strStatus !== 'PENDING') {
      return NextResponse.json(
        { error: `Only PENDING requests can be run (current status: ${request.strStatus}).` },
        { status: 400 }
      );
    }
    ...
    await query(
      "UPDATE dbo.TBL_USER_REQUEST SET strStatus = 'PROCESSING', dtStartedAt = SYSDATETIME(), strErrorMessage = NULL WHERE intId = @id",
      [{ name: 'id', type: sql.UniqueIdentifier(), value: id }]
    );
