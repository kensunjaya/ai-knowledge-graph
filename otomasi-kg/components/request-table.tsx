...
export function RequestTable({ requests, setRequests, onActionTriggered }: RequestTableProps) {
  ...
  const handleRun = async (requestId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        alert(data.error || 'Failed to run process.');
  ...
  const handleRetry = async (requestId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(err.error || 'Failed to retry request.');
        onActionTriggered(); // revert/refetch
      } else {
        onActionTriggered();
      }
  ...
  const handleRegenerate = async (requestId: string) => {
    if (!confirm('Are you sure you want to regenerate this Knowledge Graph? This will create a new cloned request using the same configuration.')) {
      return;
    }
    setActionLoadingId(requestId);
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        alert(err.error || 'Failed to regenerate request.');
      } else {
        onActionTriggered();
      }
    } catch (err) {
